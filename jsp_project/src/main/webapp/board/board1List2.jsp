...
String search = "";
if(    s_column != null && s_word != null
	&& s_column.equals("") && s_word.equals("")) {
	search = s_column + " like '%"+s_word+"%' ";
} else {
	search = " 1=1 ";
}
...
String sql0 = "select count(*) as total from board1 where "+search;
...
<!-- 본문 START -->
			...
			<div style="width:600px;margin-top:10px;text-align:center;">
				...
				%>
					<a href="<%=request.getRequestURI() %>?viewPage=<%=i%>&s_column=<%=s_column%>&s_word=<%=s_word%>"><%=i %></a>
				<%	
				}
				%>
			</div>
